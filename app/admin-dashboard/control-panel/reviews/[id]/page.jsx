...
const SingleReview = () => {
  ...
  const fetchReview = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const res = await fetch(`/api/admin/control-panel/reviews/${reviewId}`);
  ...
  const handleDelete = async () => {
    try {
      ...
      router.push('/admin-dashboard/control-panel');
    } catch (err) {
  ...
  if (!review) {
    return (
      <div className="p-6 flex flex-col items-center justify-center h-[70vh]">
        ...
        <Button onClick={() => router.push('/admin-dashboard/control-panel/reviews')}>
          Back to Reviews
        </Button>
      </div>
    );
          ...
          <CardContent>
            ...
            <div>
              ...
              <div className="text-sm space-y-1">
                <p className="font-semibold">{review.foodItem.provider.businessName}</p>
                <p className="text-gray-600">{review.foodItem.provider.address}</p>
                <p className="text-gray-600">{review.foodItem.provider.phoneNumber}</p>
              </div>
            </div>
          </CardContent>
        </Card>
